...
This project watches for incoming invoices (Gmail attachments or a Drive folder), extracts structured data with Claude, logs every invoice to the `Invoice_Log` Google Sheet, validates it against the `Vendor_Master` Google Sheet, and routes failures to a human instead of guessing.
...
2. `scripts/extract_invoice.py` — Claude reads the PDF/image and returns JSON (the schema is the `EXTRACTION_PROMPT` constant in that file), including a best-effort `category` from AP's fixed `AP_CATEGORIES` list (`config.py`) — null if nothing genuinely fits (e.g. a grocery receipt)
